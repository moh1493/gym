...
class AccountAssetCategory(models.Model):
    ...
    account_depreciation_id = fields.Many2one('account.account',
                                              ...
                                              required=True, domain=[('account_type','not in',('asset_receivable','liability_payable','asset_cash','liability_credit_card')),
                                                                             ('deprecated', '=', False)],
        ...
        for line in self:
            ...
            for move_line in move.line_ids:
                if move_line.account_id.id == move_line_1['account_id']:
                    move_line.write({'debit': move_line_1['debit'], 'credit': move_line_1['credit']})
            ...
            if move.line_ids.filtered(lambda x:x.name == 'Automatic Balancing Line'):
                print('unlink',move.line_ids.filtered(lambda x:x.name == 'Automatic Balancing Line'))
